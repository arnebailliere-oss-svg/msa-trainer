...
## Done
- E4-20 · **Mobile layout fixed** (user report from an iPhone, 2026-09-04): the dashboard's topic and mode grids had no explicit column track on phones, so the auto track took the cards' min-content width (the truncated titles) and Chromium widened the layout viewport to 614 px — the page zoomed out and "swam". `grid-cols-1` (minmax(0,1fr)) on both grids fixes it; explanation sections scroll horizontally for long formulas. New guard `e2e/overflow.spec.ts` (mobile) asserts `innerWidth ≤ device width` and no horizontal scroll on start, home, topic, session, eule, hilfe, overview and passage pages. Also: fraction/number/term inputs get on-screen keys (/, −, x, ², …) because phone keypads have no slash or minus
- E8 · **Nachweis-Modell, Tagesplan, Prüfungsreife** (2026-09-04) — mastery is now evidence, not accumulation. `core/levels.ts`: a topic's level is recomputed from its last 6 attempts — Neu → Angefangen → Geübt (4 of 6) → Sicher (5 of 6, ≥ 2 at difficulty ≥ 3, on 2 days) → Prüfungsfest (correct exam-level answer ≥ 3 days after Sicher); idle > 14 days shows as Geübt with a check due. `core/mastery.ts` projects the level onto masteryScore [0, .2, .6, .85, 1] so Ampel, Schnelltraining selection and every view keep working; no student loses progress. `core/plan.ts`: Tagesplan (mode PLAN, 12 tasks, remainder of the day's goal) = due checks → repairs → next ladder topics in topics.json order with exam-relevant (priority ≥ 2) first → keep-warm; the session consumes slots in order, repair mode still overrides. `core/readiness.ts`: Prüfungsreife = priority-weighted share of topics ≥ Sicher, ready at ≥ 85 % + all ⭐ (priority 3) topics Sicher + one passed Prüfungs-Modus day (≥ 8 tasks, ≥ 60 %), with an estimate of remaining tasks/days. All 74 leaf topics carry `priority` 1–3 from the curriculum map. UI: dashboard hero = Prüfungsreife ring + estimate, primary card „📅 Heute: 12 Aufgaben" with progress, topic rows show the level and ⭐; feedback shows the level and „⬆ Aufgestiegen!"; Topic/Overview show levels; help chapter rewritten. Attempts now store difficulty + mode. 22 new unit tests (151 total), e2e `plan.spec.ts`; ALGORITHM.md and CLAUDE.md updated. Budget: ≈ 9 tasks per topic, ≈ 660 for all three subjects ≈ 8 weeks at 12/day
...
## Decisions
- 2026-09-04 · **Rights posture (alpha, not for sale):** user decision to keep the content as is. Overlap scan against the Prüfungshefte e-books found no long verbatim runs in lessons/primers (rules and methods only, attributed „Adaptiert nach …“); verbatim material is limited to the official exam passages 2022–2024 (people/tours, blog posts, Voluntourism article, German mediation texts, some adapted from press). **Revisit before any commercial release or wider rollout:** paraphrase/shorten exam passages, ask SenBJF and pruefungshefte.de for permission, rewrite the two publisher-authored snippets (Harry-Styles Musterlösung in L_EN_WRITING, office-note example in P_EN_READING).
