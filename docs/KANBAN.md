...
- E7-6 · **Primer pedagogy rewrite (EN first)**: user review 2026-09-04 — the EN Eulen-Lektionen (`P_EN_READING`, `P_EN_EMAIL`, partly `P_EN_ZEITFORMEN`) restate the lesson's strategy tips in condensed form instead of building knowledge from zero. Rewrite rule: one new idea per board, concrete example *before* the rule, vocabulary explained before use, a worked example with the thinking shown, then the student tries one; the quiz tests what the boards built. Start with `P_EN_READING` (13 boards). · M
